        ...
        if (FILE * file = fopen(data_file.c_str(), "rb"))
		{
			...
			if (TDataPlaneIterator * iter = dataPlane.CreateIterator())
			{
				...
				while (iter->Next())
				{
                    ...
                    brush.setColor(QColor(gr,0,0,128));
                    r->setBrush(brush);
            ...
            int n = 0;*/
            const std::vector<TDataPlaneElement>&	data = temp.Data;
            double deltaX = (6178618.16+6177068.35)*0.5;
            double deltaY = (37413798.94+37415078.40)*0.5;
              for (COUNT i = 0, in = data.size(); i < in; i++)
              {
                const TDataPlaneElement& elem = data[i];
          ...
          while (iter->Next())
          {
            ...
            bricksColors << valueToColorConverter(elem.Value);
            bricks << br;
          }
          delete iter;
          */
